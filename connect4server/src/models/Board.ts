import { CONNECT_TO_WIN, EMPTY_BOX, TOTAL_COLUMNS, TOTAL_ROWS } from "../constants.js";
...
export class Board {
  ...
  constructor() {
    this.boardBoxes = Array.from({ length: TOTAL_COLUMNS }, () =>
      Array.from({ length: TOTAL_ROWS }, () => EMPTY_BOX)
    );
  }
  ...
  public makeMove(column: number, player: "1" | "2"): number {
    ...
    const moveRow = col.findIndex(cell => cell === EMPTY_BOX); // first empty = bottom
