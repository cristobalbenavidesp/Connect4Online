...
export default function AnimatedBoard() {
    const { board, throwChip, setHoverCol, winner, fallingChip } = useAnimatedBoard();
    return (
       <div
          className="relative flex border border-slate-600 rounded-xl mt-10"
        >
          ...
          {board.map((colArr, colIdx) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => {!isFull && throwChip(colIdx)}}
              ...
              >
                {colArr.toReversed().map((cell, rowIdx) => {
                  return (
                    ...
                    >
                      <GridCell colIdx={colIdx} rowIdx={(colArr.length - 1) - rowIdx} cell={cell as "1" | "2" | "0"} online={false}/>
                    </div>
                  );
                })}

                {(isFallingInThisCol || (isHoveringThisCol && !isFull)) && (!winner || fallingChip?.state === "falling") && <GhostChip/>}
